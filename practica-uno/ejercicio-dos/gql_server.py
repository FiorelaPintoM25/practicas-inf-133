...
class DeleteEstudiante(Mutation):
    ...
    def mutate(root, info, id):
        for i, planta in enumerate(plantas):
            if planta.id == id:
                plantas.pop(i)
                return DeleteEstudiante(planta=planta)
        return None

class Mutations(ObjectType):
    crear_planta = CrearPlantas.Field()
    delete_planta = DeleteEstudiante.Field()

plantas = [
    Planta(
        id=1,nombre="Rosas", especie="Gardenia", edad=5,altura=3.0,frutos=6.5
    ),
    Planta(id=2,nombre="margarita", especie="Gardenia", edad=10,altura=8.0,frutos=10.5),
]
